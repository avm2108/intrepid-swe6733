 ...
 * @param {string type} props.type - The type of link to render (green or default)
 * @param {*} props.children - The text or elements to display within the link 
 * @returns {JSX.Element} <CustomLink />
 */
export default function CustomLink(props) {
        return (
            <Link className={(props.type === "green") ? `${styles.Link} ${styles.GreenLink}` : `${styles.Link}`} to={props.to} style={props.style}>
            {props.children}
        </Link>
    );
}
